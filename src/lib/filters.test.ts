...
describe('request exploration', () => {
  it('generates a stable dataset with a visible checkout incident', () => {
    ...
    const incident = events.filter(
      (e) => e.sequence >= 13500 && e.sequence < 15000 && e.service === 'checkout',
    );
  ...
  it('round-trips filters and rejects malformed URL values', () => {
    const filters = {
      ...
      slow: true,
      sort: 'slowest' as const,
      range: 29826042,
    };
    expect(readFilters(writeFilters(filters))).toEqual(filters);
    expect(readFilters('?service=unknown&status=503&minute=NaN&sort=random')).toEqual(
      DEFAULT_FILTERS,
    );
  ...
  it('handles empty statistics and exact waterfall duration', () => {
    ...
    const event = createEvent(57);
    const spans = spansFor(event);
    expect(spans.reduce((n, s) => n + s.duration, 0)).toBe(event.duration);
    expect(
      spans.every((s, i) => i === 0 || s.start === spans[i - 1].start + spans[i - 1].duration),
    ).toBe(true);
  });
});
