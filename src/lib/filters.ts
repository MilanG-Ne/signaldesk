...
export type Filters = { query: string; service: Service | 'all'; status: 'all' | 'success' | 'client' | 'server'; slow: boolean; sort: 'newest' | 'slowest'; range: number | null };
export const DEFAULT_FILTERS: Filters = { query: '', service: 'all', status: 'all', slow: false, sort: 'newest', range: null };
export function filterEvents(events: readonly RequestEvent[], filters: Filters) {
  ...
  const results = events.filter(event => {
    ...
    if (filters.range !== null && Math.floor(event.timestamp / 60_000) !== filters.range) return false;
    const haystack = `${event.path} ${event.service} ${event.method} ${event.id} ${event.region} ${event.status}`.toLowerCase();
    return words.every(word => haystack.includes(word));
  });
  if (filters.sort === 'slowest') results.sort((a, b) => b.duration - a.duration || b.sequence - a.sequence);
  else results.sort((a, b) => b.sequence - a.sequence);
  return results;
}
export function readFilters(search: string): Filters {
  const p = new URLSearchParams(search);
  const service = p.get('service'); const status = p.get('status'); const range = p.get('minute');
  return { query: (p.get('q') ?? '').slice(0, 160), service: SERVICES.includes(service as Service) ? service as Service : 'all',
    status: ['success', 'client', 'server'].includes(status ?? '') ? status as Filters['status'] : 'all',
    slow: p.get('slow') === '1', sort: p.get('sort') === 'slowest' ? 'slowest' : 'newest',
...
export function summarize(events: readonly RequestEvent[]) {
  ...
  return { errors, p95: times[Math.ceil(times.length * .95) - 1], errorRate: (errors / events.length * 100).toFixed(1) };
}
export function histogram(events: readonly RequestEvent[]) {
  ...
  for (const event of events) {
    ...
    bin.total++; if (event.status >= 500) bin.errors++; bins.set(minute, bin);
  }
