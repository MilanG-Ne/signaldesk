...
export function createEvent(sequence: number): RequestEvent {
  ...
  const chance = next();
  const status =
    chance < (incident ? 0.58 : 0.035)
      ? 503
      : chance < (incident ? 0.65 : 0.07)
        ? 404
        : method === 'POST'
          ? 201
          : 200;
  const duration = Math.round(
    18 + next() * 170 + (status >= 500 ? 600 + next() * 1400 : incident ? 500 : 0),
  );
  return Object.freeze({
    id: `req_${sequence.toString(16).padStart(8, '0')}`,
    sequence,
    timestamp: BASE_TIME + sequence * 180,
    method,
    path,
    service,
    status,
    duration,
    region: ['fra-1', 'iad-1', 'ams-1'][Math.floor(next() * 3)],
    bytes: Math.round(180 + next() * 9500),
  });
}
...
export function spansFor(event: RequestEvent) {
  const names = [
    'Edge routing',
    'Authentication',
    event.service === 'checkout' ? 'Payment service' : 'Application handler',
    'Response',
  ];
  let offset = 0;
  return [0.06, 0.13, 0.76, 0.05].map((fraction, index) => {
    const span = {
      name: names[index],
      start: offset,
      duration: index === 3 ? event.duration - offset : Math.floor(event.duration * fraction),
      failed: index === 2 && event.status >= 500,
    };
